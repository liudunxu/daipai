...
export default function SEOManagePage() {
  ...
  // 查看原始输入内容
  function viewArticleContent(kw) {
    setContentModalData({
      title: '原始输入内容',
      content: kw.keyword
    })
  ...
  return (
    <>
      ...
      {/* 内容预览弹窗 */}
      {showContentModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowContentModal(false)}>
          <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6 max-w-2xl w-full mx-4 shadow-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-white">{contentModalData.title}</h3>
              <button
            ...
            <div className="flex-1 overflow-y-auto">
              <pre className="text-white/80 text-sm whitespace-pre-wrap break-words font-mono leading-relaxed bg-white/5 rounded-xl p-4">
                {contentModalData.content}
              </pre>
            </div>
            <div className="mt-4 flex justify-end">
              <button
                onClick={() => setShowContentModal(false)}
                className="px-6 py-2 bg-white/10 text-white/80 hover:bg-white/20 rounded-lg transition-colors"
              >
                关闭
              </button>
            </div>
