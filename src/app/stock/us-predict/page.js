...
export default function USStockPredictPage() {
  ...
  useEffect(() => {
    const fetchStocks = async () => {
      try {
        ...
        setError(null)
        const response = await fetch('/api/stock/stocks?zone=us')
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <footer className="mt-8 text-center">
          <div className="flex justify-center gap-4 mb-2">
            ...
            <a href="/stock/hk-predict" className="text-white/30 hover:text-white/60 text-sm transition-colors">
              ← 港股预测
            </a>
          </div>
        </footer>
